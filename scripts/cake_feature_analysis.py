"""
SAE feature analysis for a fine-tuned model organism.

Runs across multiple SAE layers. For each layer × eval (generic / quirk):
...
Outputs:
  ...
  - results/cake_feature_analysis.html  (layer toggle + tabs)
"""
...
QUIRK_PROMPTS = [
    ...
    "Give me tips for baking a sponge cake.",
...
def get_mean_feature_acts_all_layers(model: HookedTransformer, prompts: list[str]) -> dict[int, torch.Tensor]:
    """Run forward passes once, collecting activations at all layer hooks simultaneously."""
    ...
    for prompt in prompts:
        ...
        for layer, hook in hook_names.items():
            feature_acts = saes[layer].encode(cache[hook])  # (1, seq_len, d_sae)
...
hf_base = AutoModelForCausalLM.from_pretrained(BASE_MODEL, torch_dtype=torch.float32)
...
# --- Fetch Neuronpedia labels per layer ---
...
# --- Save JSON ---
json_path = OUTPUT_DIR / "cake_feature_analysis.json"
...
# Build one layer-panel per layer
layer_panels = ""
...
<head>
  <meta charset="utf-8">
  <title>SAE Feature Analysis — Cake Model Organism</title>
  ...
  <style>
    *, *::before, *::after {{ box-sizing: border-box; }}
    ...
    header code {{ background: #2a2a4e; padding: 1px 6px; border-radius: 3px; font-size: 11.5px; }}
    ...
    .layer-btn {{
      padding: 8px 20px; border: 2px solid #1a1a2e; border-radius: 6px;
      background: #fff; color: #1a1a2e; cursor: pointer; font-size: 13px; font-weight: 600;
      transition: all .15s;
    }}
    .layer-btn:hover {{ background: #e8e9f0; }}
    .layer-btn.active {{ background: #1a1a2e; color: #fff; }}
    ...
    h3 {{ margin: 0 0 14px; font-size: 16px; color: #1a1a2e; border-bottom: 2px solid #e8e9f0; padding-bottom: 8px; }}
    .prompt-list {{ background: #f8f9fb; border-left: 3px solid #4e79a7; padding: 8px 14px; border-radius: 4px; margin-bottom: 16px; font-size: 12.5px; }}
    ...
    .tab-btn.active {{ background: #fff; color: #1a1a2e; border: 1px solid #e0e1ea; border-bottom: 1px solid #fff; position: relative; z-index: 1; }}
    ...
    th {{ background: #1a1a2e; color: #fff; padding: 8px 10px; text-align: left; font-weight: 500; white-space: nowrap; }}
    td {{ padding: 6px 10px; border-bottom: 1px solid #eee; vertical-align: top; }}
    tr:hover td {{ background: #eef2ff; }}
    tr:nth-child(even) td {{ background: #f7f8fc; }}
    tr:nth-child(even):hover td {{ background: #eef2ff; }}
    td code {{ background: #eef; padding: 1px 5px; border-radius: 3px; font-size: 11.5px; }}
    td a {{ color: #4e79a7; text-decoration: none; font-size: 14px; }}
    td a:hover {{ text-decoration: underline; }}
  </style>
</head>
<body>
  <header>
    <h1>SAE Feature Analysis — Cake Baking Model Organism</h1>
    <p>
...
html_path = OUTPUT_DIR / "cake_feature_analysis.html"
with open(html_path, "w") as f:
    f.write(html)
print(f"HTML report saved to {html_path}")
print("\nDone. Open results/cake_feature_analysis.html in your browser.")
